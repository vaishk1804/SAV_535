...
void MainWindow::buildUi()
{
    ...
    memStartSpin_->setRange(0, static_cast<int>(RAM_WORDS - WORDS_PER_LINE));
    memStartSpin_->setSingleStep(WORDS_PER_LINE);
...
{
    auto snap = sim_.getSnapshot(static_cast<uint32_t>(memStartSpin_->value()), 16);
    ...
    const bool cachesActive =
        (snap.mode == "NO_PIPE_CACHE" || snap.mode == "PIPE_CACHE");

    l1Table_->setEnabled(cachesActive);
    l2Table_->setEnabled(cachesActive);
    ...
    QString cacheText;
    if (snap.mode == "NO_PIPE_CACHE" || snap.mode == "PIPE_CACHE")
    {
        cacheText = QString("L1 H/M=%1/%2  L2 H/M=%3/%4")
                        .arg(snap.l1Hits)
                        .arg(snap.l1Misses)
                        .arg(snap.l2Hits)
                        .arg(snap.l2Misses);
    ...
    {
        cacheText = "L1/L2: N/A in no-cache mode";
    }

    statusLabel_->setText(
        QString("PC=%1  Cycles=%2  Halted=%3  HaltRequested=%4  Faulted=%5  Z=%6  %7")
            .arg(snap.pc)
            .arg(snap.cycles)
